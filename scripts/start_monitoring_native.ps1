...
function Ensure-Downloaded {
    ...
    Invoke-DirectWebRequest -Url $Url -OutFile $Destination
}
...
function Ensure-MonitoringConfigs {
    ...
    $dashboardSource = Join-Path $ProjectRoot 'ops\monitoring\grafana\dashboards\lawvoice-observability.json'
    ...
    $dashboardTarget = Join-Path $GrafanaRuntimeDir 'dashboards\lawvoice-observability.json'
    ...
    Copy-IfChanged -Source $dashboardSource -Destination $dashboardTarget
