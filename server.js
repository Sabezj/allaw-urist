...
// ---- Python Search Proxy (guarded) ---------------------------------
const USE_PY_SEARCH = process.env.USE_PY_SEARCH === 'true';
...
if (USE_PY_SEARCH) {
  ...
  app.get('/api/products/search', (req, res) => forwardSearch(req, res, '/v1/products/search'));
  app.get('/api/products', (req, res) => forwardSearch(req, res, '/v1/products'));
  app.get('/api/products/categories', (req, res) => forwardSearch(req, res, '/v1/products/categories'));
}
// --------------------------------------------------------------------
...
const requireAuth = config.get('auth.devNoAuth') ? (req, res, next) => next() : verifyJWT;
...
async function searchKnowledgeChunks(queryText, limit = 6) {
  ...
  if (vectorExtensionAvailable) {
    try {
      ...
      logger.warn({ err }, 'Knowledge semantic search failed, falling back to FTS');
    }
  }
  ...
  try {
    ...
    return normalizeKnowledgeHitRows(rows);
  } catch (err) {
    logger.error({ err }, 'Knowledge fallback search failed');
    return [];
  }
}
...
// Tail last 100 log lines (requires X-API-Key header)
app.get('/api/logs', async (req, res) => {
  ...
  try {
    ...
    res.type('text/plain').send(lines.slice(-100).join('\n'));
  } catch (err) {
...
app.get('/api/agent-console', (req, res) => {
  res.setHeader('Content-Type', 'text/event-stream');
  res.setHeader('Cache-Control', 'no-cache');
  res.setHeader('Connection', 'keep-alive');
  res.flushHeaders?.();
...
app.get('/api/session', async (req, res) => {
  try {
    if (config.get('openai.mock')) {
      ...
      logger.info('🔧 Using mock OpenAI session data');
    ...
    logger.info(
      `✅ Ephemeral token generated successfully for model: ${sessionData.model}`
    );
    res.json(sessionData);
  } catch (error) {
    const status = error.response?.status || 500;
    const details = error.response?.data || error.message;
    logger.error('❌ Error generating ephemeral token:', details);
...
app.post('/api/realtime/sdp', express.text({ type: ['application/sdp', 'text/plain'], limit: '2mb' }), async (req, res) => {
  try {
    ...
    logger.error('❌ Error proxying realtime SDP exchange:', error?.message || error);
    res.status(502).json({
...
app.get('/api/products/search', async (req, res) => {
  ...
  const sanitizedQuery = q
    ...
    .replace(/\s+/g, ' ')
    .trim();
  logger.info({ msg: '🛢️ DB search request', query: sanitizedQuery, limit: parsedLimit });
  ...
  try {
    ...
    logger.info({ msg: '🧾 DB search completed', query: sanitizedQuery, results: totalCount });
...
app.post('/api/classify-intent', requireAuth, async (req, res) => {
  const { transcript, mode } = req.body;
  if (!transcript) return res.status(400).json({ error: 'Missing transcript' });
  logger.info({ msg: '🎯 Intent detection request', transcript, mode: mode || 'auto' });
  try {
    const { toolCall, confidence, meta } = await classifyIntent(transcript, { mode });
    logger.info({ msg: '🎯 Intent detection result', toolCall, confidence, meta });
    res.json({ toolCall, confidence, meta: meta || null });
  } catch (err) {
    const dev = process.env.NODE_ENV !== 'production';
    const status  = err?.response?.status || 500;
    const details = err?.response?.data   || err.message;
    logger.error({ msg:'Intent classification failed', status, details });
    res.status(status).json({
...
app.post('/api/knowledge/search', requireAuth, async (req, res) => {
  ...
  try {
    const hits = await searchKnowledgeChunks(queryText, parsedLimit);
    res.json({
      query: queryText,
      hits,
      count: hits.length
    ...
    logger.error({ err, queryText }, 'Knowledge search failed');
    res.status(500).json({ error: 'Knowledge search failed' });
  }
});
app.post('/api/action-plan', requireAuth, async (req, res) => {
  ...
  const knowledgeHits = await searchKnowledgeChunks(queryForKnowledge, knowledgeLimit);
  ...
  try {
    ...
    logger.error({ err }, 'Action plan generation failed');
