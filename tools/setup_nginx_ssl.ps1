...
Write-Host "Testing: http://$DOMAIN" -ForegroundColor Gray
$httpTest = & $SSH_EXE -i $SSH_KEY $VPS_USER@$VPS_IP "curl -s -o /dev/null -w '%{http_code}' http://localhost"
...
$certExists = & $SSH_EXE -i $SSH_KEY $VPS_USER@$VPS_IP "test -f /opt/certs/allaw-urist.ru/fullchain.pem && test -f /opt/certs/allaw-urist.ru/privkey.pem && echo 'yes' || echo 'no'"

if ($certExists -eq "yes") {
    Write-Host "✓ SSL certificates found at /opt/certs/allaw-urist.ru/" -ForegroundColor Green
    
    # Show certificate info
    Write-Host "`nCertificate details:" -ForegroundColor Gray
    & $SSH_EXE -i $SSH_KEY $VPS_USER@$VPS_IP "openssl x509 -in /opt/certs/allaw-urist.ru/fullchain.pem -noout -subject -dates 2>/dev/null || echo 'Could not read certificate details'"
} else {
    Write-Host "✗ SSL certificates not found at /opt/certs/allaw-urist.ru/" -ForegroundColor Red
    Write-Host "Expected files:" -ForegroundColor Yellow
    Write-Host "  /opt/certs/allaw-urist.ru/fullchain.pem" -ForegroundColor Gray
    Write-Host "  /opt/certs/allaw-urist.ru/privkey.pem" -ForegroundColor Gray
}
...
if ($certExists -eq "yes") {
    ...
    Write-Host "✗ SSL certificates missing - check /opt/certs/allaw-urist.ru/" -ForegroundColor Red
}
